...
- **Eyebrow scale-up + VIEW ALL → `.micro`** (Luis 2026-05-25 dashboard vet): bumped "UPCOMING MAINTENANCE" eyebrow `Type.Label.xs` (10pt) → `Type.Label.sm` (13pt) for more visual weight against the row content. VIEW ALL → button correspondingly downsized `.small` → `.micro` to keep it inline-secondary against the now-bigger eyebrow rather than competing.
